...
from renode_bridge import RenoneBridge, detect_firmware
...
# ─── Step 3: Detect firmware ELF ──────────────────────────────────────────────
elf_path = detect_firmware()
if not elf_path:
    print("ERROR: No firmware ELF found. Halting.")
    sys.exit(1)
...
# ─── Step 4: Run Renode bridge ────────────────────────────────────────────────
print("=" * 60)
print("Launching RenoneBridge ...")
print("=" * 60)

t_start = time.time()
bridge = RenoneBridge(elf_path=elf_path)
...
# ─── Step 5: uart-reader agent — print all UART events ───────────────────────
...
print(f"  {'------':>6}  {'----------':>10}  {'---------':>9}  {'-----------':>11}")
for sn in snapshots:
    flag = " <-- ABOVE 10% THRESHOLD" if sn.si_stance_pct > 10.0 else ""
...
si_above_threshold = final_si > 10.0
print(f"  SI > 10%       : {'YES -- pathological asymmetry confirmed' if si_above_threshold else 'NO -- below clinical threshold'}")
print("=" * 60)

# Check for 0-step failure — escalation trigger per Standing Order
...
# Write results to a file for the plotter agent to use
import json
results = {
    "profile": profile_key,
